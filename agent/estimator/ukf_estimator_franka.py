...
class UKFEstimator(object):
    def __init__(self, spec, model):
        ...
        self.kp                  = spec["kp"]               #naive p-controller gain 
        self.kv                  = spec["kv"]               #naive d-controller gain 
        self.model               = model
        self.dt                  = spec["time_sample"]      #dt val
    ...
    def estimate(self, u,sensor_data):
        ...
        other_agent_state = 0*self.posterior_state  # this is just a placeholder
        ...
        for i in range((2*N)+1):
                v                     = X[:,i]
                vel                   = np.array(u);
                z                     = v.ravel()  + vel.ravel()*self.dt ;
                priori_pos_sigma[:,i] = z.ravel().reshape((3,))


        all_measurements         = np.vstack(np.ravel(sensor_data["state_sensor"]["state"]))
        z_meas                   = all_measurements[0:3]
        ...
        print(est_param["ego_state_est"])
        print(est_param["other_state_est"])
        # print("otherstate")
