    ...
    def _solve(self, T, p_i, s_i, v, vis, vis_obs, conflict):
        ...
        for j in range(len(query_obs)):
            for k_i in range(len(query_obs[j])):
                ...
                # if agent sphere intersects multiple obs sphere from path, just treat as one
