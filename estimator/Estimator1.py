...
class Zero_Estimator(Estimator_Base):
	...
	def estimate(self, dt, x, goal_x, est_params):
...
class Extended_Kalman_Filter(Estimator_Base):
	...
	def __init__(self,estimator_params,model):
		...
		self.model  = model
		self.Rww    = estimator_params['Rww']
		self.Rvv    = estimator_params['Rvv']
	...
	def estimate(self,posterior_pos,posterior_state_cov,z_meas):
		priori_pos               = robot_model.forward_propagate_dynamics(posterior_state)      				; # need to call dynamics of the paritcular robot model
		A                        = robot_model.linearization_forward_dynamics(posterior_state)  				; # need to call dynamics of the paritcular robot model
		priori_cov               = np.matmul(np.matmul(A,posterior_state_cov),np.transpose(A)) + self.Rww      	;   
		n                        = len(posterior_pos)                                           				;
		I                        = np.identity(n)                                               				;
		H                        = robot_model.linearization_measurement_function(priori_pos)   				; # need to call dynamics of the paritcular robot model
		innovation               = z_meas - robot_model.measurement_function(priori_pos)        				; # need to call dynamics of the paritcular robot model
		S                        = np.matmul(np.matmul(H,priori_cov),np.transpose(H)) + self.Rvv  				;
		Sinv                     = np.linalg.inv(S)																;
		K                        = np.matmul(priori_cov,np.transpose(H))*Sinv                   				;
