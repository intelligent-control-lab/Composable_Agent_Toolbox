class CBF_hV:
    ...
    def ub(self, x):
        return (self.alpha[1] * self.hV_dot(x) + self.alpha[0] * self.hV(x) 
          - self.idm.lamb(x['pH'][0], x['pR'][0], x['vH'][0], x['vR'][0], x['aH'][0])) \
    / self.idm.df_dvR(x['pH'][0], x['pR'][0], x['vH'][0], x['vR'][0])
    
    def _alpha(self, x):
        c1 = max(0, -self.hV_dot(x) / self.hV(x)) + self.c_min
        c2 = max(0, -(0 + c1 * self.hV_dot(x)) 
                / (self.hV_dot(x) + c1 * self.hV(x))) + self.c_min
        return [c1 + c2, c1 * c2]