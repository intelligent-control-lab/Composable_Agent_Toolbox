...
class DQN_Agent():
    ...
    def new_env_instance(self):
        ...
        # randomize agent location and goal if None
        for agent_id in env_spec['agent_env_spec'].keys():
            # assign init location
            if env_spec['agent_env_spec'][agent_id]['spec']['init_x'] is None:
                if agent_id == 'robot':
                    env_spec['agent_env_spec'][agent_id]['spec']['init_x'] = \
                        [np.random.rand()*100, np.random.rand()*100, 0, 0]
                else:
                    robot_start_pos = np.array(env_spec['agent_env_spec']['robot']['spec']['init_x'][:2])
                    robot_goal_pos = np.array(env_spec['world']['spec']['agent_goal_lists']['robot'][0])
                    mid_pt = (robot_start_pos + robot_goal_pos) / 2.0
                    mid_pt += (np.random.rand(2) * 20 - 10)
                    env_spec['agent_env_spec'][agent_id]['spec']['init_x'] = \
                        list(mid_pt) + [0, 0]
                
            # assign goal
            if env_spec['world']['spec']['agent_goal_lists'][agent_id] == 'random':
                env_spec['world']['spec']['agent_goal_lists'][agent_id] = \
                    [[np.random.rand()*100, np.random.rand()*100]]
            elif env_spec['world']['spec']['agent_goal_lists'][agent_id] == 'static':
                env_spec['world']['spec']['agent_goal_lists'][agent_id] = \
                    [env_spec['agent_env_spec'][agent_id]['spec']['init_x'][:2]]
            elif env_spec['world']['spec']['agent_goal_lists'][agent_id] == 'near':
                env_spec['world']['spec']['agent_goal_lists'][agent_id] = \
                    [env_spec['agent_env_spec'][agent_id]['spec']['init_x'][:2]]
                env_spec['world']['spec']['agent_goal_lists'][agent_id][0][0] += np.random.rand()*20
                env_spec['world']['spec']['agent_goal_lists'][agent_id][0][1] += np.random.rand()*20
    ...
    def env_step(self, env_instance, agents, measurement_groups, action, dt):
        ...
        done = (self.step_env_instance >= 200)
...
def parse_arguments():
    ...
    parser.add_argument('--grad_norm', action="store", type=float, default="1.0")
    ...
    parser.add_argument('--C', action="store", type=int, default="100")
    ...
    parser.add_argument('--checkpt_freq', action="store", type=int, default="50")
...
def main(args):
    ...
    if args.demo:
        ...
        args.exp_name = "{}_{}_C{}_H{}_Nm{}_lr{}".format(
            args.env_name, env_spec['world']['spec']['agent_goal_lists']['human'],
