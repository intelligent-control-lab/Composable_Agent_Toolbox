...
def plot_data(data, xaxis='Epoch', value="AverageEpRet", 
 ...
 (0.09019607843137255, 0.7450980392156863, 0.8117647058823529)]
    ...
    sns.tsplot(data=data, time=xaxis, value=value, unit="Unit", condition=condition, ci='sd', alpha=1.0, **kwargs)
    """
...
def get_datasets(logdir, condition=None):
    ...
    for root, _, files in os.walk(logdir):
        if 'progress.txt' in files:
            ...
            if ("ppo" in condition1) and ("ppo_lag" not in condition1)  and ("ppo_adam" not in condition1):
                condition3 = "PPO"
            elif "ppo_lag" in condition1:
                condition3 = "PPO-Lagrangian"
            elif "ppo_adam" in condition1:
                condition3 = "PPO-ISSA"
            elif "cpo" in condition1:
                condition3 = "CPO"
            ...
            try:
                exp_data = pd.read_table(os.path.join(root,'progress.txt'))
                # exp_data = pd.read_table(os.path.join(root,'progress.txt'))[0:50]
            except:
...
def metrics_print(data):
    length = len(data[0]["AverageEpRet"])
    result = {}
    for exp in data:
        
        method_name = exp["Condition1"][0]

        if method_name not in result.keys():
            result[method_name] = {}
            result[method_name]["Return"] = []
            result[method_name]["Violation"] = []
            result[method_name]["CostRate"] = []

        if "admaba" in method_name:
            print(exp["AverageEpCost"])
        for i in range(5):
            result[method_name]["Return"].append(exp["AverageEpRet"][length-1-i])
            result[method_name]["Violation"].append(exp["AverageEpCost"][length-1-i])
        result[method_name]["CostRate"].append(exp["CostRate"][length-1])
    n_r = 0
    n_v = 0
    n_c = 0
    for method in result.keys():
        ...
        print("\tRuturn = ", np.mean(result[method]["Return"]))
        print("\tViolation = ", np.mean(result[method]["Violation"]))
        print("\tCostRate = ", np.mean(result[method]["CostRate"]))

        if ("ppo" in method) and ("ppo_lag" not in method)  and ("ppo_adam" not in method):
            n_r =  np.mean(result[method]["Return"])
            n_v =  np.mean(result[method]["Violation"])
            n_c =  np.mean(result[method]["CostRate"])
    ...
    for method in result.keys():
        ...
        print("\tRuturn = ", np.mean(result[method]["Return"])/n_r)
        print("\tViolation = ", np.mean(result[method]["Violation"])/n_v)
        print("\tCostRate = ", np.mean(result[method]["CostRate"])/n_c)
...
def main():
    ...
    parser.add_argument('--hidelegend', '-hl', action='store_false')
